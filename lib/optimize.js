...
const task = {
  ...
  watch(iEnv) {
    ...
    if (cache.app) {
      let iDevMiddle = devMiddleware(compiler, {
        noInfo: true,
      ...
      let iHotMiddle = hotMiddleware(compiler, {
        publicPath: config.output.publicPath,
        log: false
      });

      if (cache.platform === 'koa') {
        iDevMiddle = fn.makeKoaMiddleware(iDevMiddle);
        iHotMiddle = fn.makeKoaMiddleware(iHotMiddle);
      }

      cache.app.use(iDevMiddle);
      cache.app.use(iHotMiddle);
    } else {
      compiler.watch({
        aggregateTimeout: 1000
...
const wOpzer = function (iConfig) {
  ...
  opzer.initServerMiddleWare = function (app, iEnv, platform) {
    cache.app = app;
