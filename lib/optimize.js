...
const fn = {
  ...
  buildWConfig: function (iEnv) {
    ...
    if (cache.root) {
      const wPath = util.path.join(cache.root, 'webpack.config.js');
...
const task = {
  async preCheck () {
    ...
    if (fs.existsSync(pkgPath)) {
      ...
      if (checkMap) {
        Object.keys(checkMap).some((key) => {
          if (
            !nodePathExists ||
            util.compareVersion(checkMap[key], getModuleVersion(key)) !== 0
          ) {
            needInstall = true;
            return true;
          }
    ...
    if (needInstall) {
      await extOs.runCMD('npm install', cache.root);
    }
    // - 版本校验
  },
  all(iEnv, done) {
