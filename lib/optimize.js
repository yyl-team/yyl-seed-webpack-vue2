...
const task = {
  ...
  watch(iEnv) {
    ...
    if (cache.app) {
      const iDevMiddle = devMiddleware(compiler, {
        noInfo: true,
      ...
      const iHotMiddle = hotMiddleware(compiler, {
        publicPath: config.output.publicPath,
        log: false
      });

      if (cache.platform === 'koa') {
        cache.app.use(async (ctx, next) => {
          await iDevMiddle(ctx.reg, {
            end: (content) => {
              ctx.body = content;
            },
