...
const init = (type, targetPath) => {
  ...
  const task04 = new Promise((next, reject) => {
    ...
    const gitIgnorePath = path.join(EXAMPLE_PATH, '.gitignore');
    const npmIgnorePath = path.join(EXAMPLE_PATH, '.npmignore');
    let ignorePath;

    if (fs.existsSync(gitIgnorePath)) {
      ignorePath = gitIgnorePath;
    } else if (fs.existsSync(npmIgnorePath)) {
      ignorePath = npmIgnorePath;
    }

    copyMap[ignorePath] = path.join(targetPath, '.gitignore');
