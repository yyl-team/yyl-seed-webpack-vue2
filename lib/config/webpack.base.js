...
const map2Babel = function (str) {
  const nodeModulePath1 = path.join(__dirname, '../../node_modules');
  const nodeModulePath2 = path.join(__dirname, '../../../node_modules');
  const path1 = path.join(nodeModulePath1, '@babel');
  if (fs.existsSync(path1)) {
    return util.path.join(nodeModulePath1, str);
  } else {
